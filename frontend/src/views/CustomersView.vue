...
import { PaymentTerm, type Customer } from '@/types'

const auth = useAuthStore()
const notifications = useNotificationStore()
const { currency } = useFormat()

const list = useResourceList<Customer>(customersApi.list, { initialFilters: { active: true } })
...
function openEdit(customer: Customer) {
  ...
  Object.assign(form, {
    ...
    paymentTerm: customer.paymentTerm as number,
...
async function save() {
  ...
  errors.value = {}
  try {
    const payload = { ...form, paymentTerm: Number(form.paymentTerm) }
    if (editing.value) {
      await customersApi.update(editing.value.id, payload as Partial<Customer>)
      notifications.success('Cliente actualizado.')
    } else {
      await customersApi.create(payload as Partial<Customer>)
      notifications.success('Cliente registrado.')
    }
...
const confirming = ref<Customer | null>(null)
...
async function confirmToggle() {
  ...
  try {
    ...
    await customersApi.setActive(target.id, !target.isActive)
    notifications.success(target.isActive ? 'Cliente inactivado.' : 'Cliente reactivado.')
...
<template>
  <div>
    <PageHeader title="Clientes" description="Empresas y personas a las que se les vende.">
      <template #actions>
        <BaseButton v-if="auth.canWrite('customers')" variant="primary" @click="openCreate">
          Nuevo cliente
        </BaseButton>
      </template>
    </PageHeader>
    ...
    <DataTable
      ...
      empty-message="Registre su primer cliente para empezar a facturarle."
    ...
    >
      ...
      <template #terms="{ row }">
        ...
        <small v-if="row.creditLimit > 0" class="muted block">Límite {{ currency(row.creditLimit) }}</small>
      </template>
      ...
      <template #actions="{ row }">
        <div class="row-actions">
          <BaseButton size="sm" @click="openEdit(row)">
            {{ auth.canWrite('customers') ? 'Editar' : 'Ver' }}
          </BaseButton>
          <BaseButton v-if="auth.canWrite('customers')" size="sm" variant="ghost" @click="confirming = row">
    ...
    <BaseModal
      ...
      subtitle="Los datos de crédito determinan cómo se registran sus ventas."
    ...
    >
      <form id="customer-form" class="form-grid" @submit.prevent="save">
        ...
        <BaseField v-slot="{ id, invalid }" label="Cédula jurídica o física" :error="errors.taxId">
          <input :id="id" v-model="form.taxId" class="control" :class="{ 'is-invalid': invalid }" maxlength="50" />
        </BaseField>
        ...
        <BaseField
          ...
          :hint="Number(form.paymentTerm) === PaymentTerm.Cash ? 'Solo aplica a clientes de crédito.' : undefined"
        >
          <input
            :id="id"
        ...
        <BaseField v-slot="{ id, invalid }" label="Límite de crédito" hint="0 = sin límite definido." :error="errors.creditLimit">
          <input
            :id="id"
    ...
    <ConfirmDialog
      :open="!!confirming"
      :title="confirming?.isActive ? 'Inactivar cliente' : 'Reactivar cliente'"
      :message="
        confirming?.isActive
          ? `${confirming?.name} dejará de aparecer en nuevas ventas, pero su historial se conserva íntegro.`
          : `${confirming?.name} volverá a estar disponible para nuevas operaciones.`
      "
